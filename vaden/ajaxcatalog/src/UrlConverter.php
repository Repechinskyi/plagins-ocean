...
class UrlConverter {
  ...
  public function paramsGenerate($url) {
    ...
    if ($entity) {
      ...
      $cache_tags = [
        config('app.name'),
        config('app.name') . 'ajax_catalog_url_converter',
      ];
  ...
  public function urlGenerate($params, $with_pagination = false) {
    ...
    if ($this->short_url) {
      ...
      if (isset($this->origin_entity['rout_group'])) {
        $sub_params['00-route-group'] = [
          'type' => 'route-group',
          'value' => $this->origin_entity['rout_group'],
        ];
        ksort($sub_params);
      }
    }
        ...
        if (isset($this->values[$url_prefix][$alias])) {
          $param = [
            ...
            'value' => [
              $settings['table_column'],
              $settings['operator'],
              $this->values[$url_prefix][$alias]['value'],
              $url_prefix,
            ],
          ];
        }
  ...
  */
  public function isAliasPagination($alias) {
    ...
    if (!empty($alias) && count($alias) == 2 && $alias[0] == 'p' && is_numeric($alias[1])) {
      $param = [
        'type' => 'pagination',
        'key' => 'curr_page',
    ...
    if (!empty($alias) && count($alias) == 2 && $alias[0] == 'sm' && is_numeric($alias[1])) {
