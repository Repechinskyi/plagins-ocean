...
class ResultsGenerator
{
    ...
    public function getFilterCounts($filter_name)
    {
        ...
        $filterIndex = isset($this->urlData->settings['filters'][$filter_name]['index_counter']) ? $this->urlData->settings['filters'][$filter_name]['index_counter'] : false;
        ...
        if($filterIndex) {
          foreach ($results as $key => $result) {
            foreach ($available_languages as $available_language) {
				$result->value = str_replace($available_language . '/', '', $result->value);
			}
            $key_arr = explode('|', $result->value);
            foreach ($key_arr as $key) {
              if($key != '') {
                $formatted_key = '%' . $key . '%';
                if(isset($valuesCount[$formatted_key])) {
                  $valuesCount[$formatted_key] += $result->count;
                } else {
                  $valuesCount[$formatted_key] = $result->count;
                }
              }
        ...
        if ($operator == 'in') {
            ...
            if($operator == 'like') {
              ...
              if($lang !== $default_language) {
                  $value = str_replace('|', '', $value);
                  if($column != 'index_product_pages.equipments' && $column != 'index_product_pages.attributes') {
                      $value = $lang . '/' .$value;
                  }

                  if($column != 'index_product_pages.country' && $column != 'index_product_pages.equipments' && $column != 'index_product_pages.attributes' && $column != 'index_product_pages.manufacturer_id' ) {
                      $value = '|' . $value . "|";
                  }
              }
              if($column == 'index_product_pages.manufacturer_id' ) {
                  $value = '%|' . $value . "|%";
              } else {
                  $value = '%' . $value . "%";
              }
