...
class LinksList extends Widgets
{
    ...
    */
    ...
    private function getCountersData()
    {
		...
		if(isset($urlData->params['rout_group']) && strpos($urlData->params['entity'], 'search_product_page') === false) {
          foreach ($urlData->params['filters'] as $key => $filter_item) {
            $str_filter = $urlData->params['filters'][$key][2];
            $check_symbol = strrpos($str_filter,'|');
            if($filter_item[0] !== 'attributes') {
              if($check_symbol) {
                $urlData->params['filters'][$key][2] = substr($str_filter,0, 2) . $urlData->params['rout_group'] . '/' . substr($str_filter,2);
              }
              else {
                $urlData->params['filters'][$key][2] = substr($str_filter,0, 1) . $urlData->params['rout_group'] . '/' . substr($str_filter,1);
              }
            }
          }
        }
