...
class SubModelIndex extends Widgets
{
    ...
    public function __construct($urlData, $widget_system_name, $widget_name)
    {
        ...
        foreach ($urlData->values[$widget_system_name] as $key => $value) {
            ...
            if ($value['parent']) {
                $alias = $this->getAlias($value['parent']);
                $alias = str_replace('%', '', $alias);
                foreach ($available_languages as $available_language) {
                  $alias = str_replace($available_language . '/', '', $alias);
                }
                $this->children[$alias][$value['alias']] = $value;
            }
        }
    ...
    */
    private function getGroupedValues()
    {
        ...
        foreach ($this->urlData->params['filters'] as $filter) {
            if ($filter[3] == $parentFilter) {
                $parentValue = str_replace('%', '', $filter[2]);
                $parentValue = str_replace('|', '', $parentValue);
            }
        }
        $parentValue2 = $this->values[$this->getAlias($parentValue)];

        $parentValue2['value'] = str_replace('%', '', $parentValue2['value']);
        $parentValue2['parent'] = str_replace('%', '', $parentValue2['parent']);
    ...
    private function getSubCategories($grouped_values, $parents)
    {
        ...
        foreach ($grouped_values as $group_val) {
            $group_val['value'] = str_replace('%', '', $group_val['value']);
            $group_val['value'] = str_replace('|', '', $group_val['value']);
    ...
    */
    ...
    private function addParentToArr($parents, $value)
    {
        ...
        if (isset($value['parent']) && $value['parent'] != null) {
            foreach ($available_languages as $available_language) {
              $value['parent'] = str_replace($available_language . '/', '', $value['parent']);
            }
    ...
    */
    ...
    private function getCountersData()
    {
        ...
        $data = with(new ResultsGenerator($urlData))->getFilterCounts($this->system_name);

        $data2 = [];
        foreach ($data as $key => $datum) {
          foreach ($available_languages as $available_language) {
            $key = str_replace($available_language .'/', '', $key);
          }
          $data2[$key] = $datum;
        }
