...
class AjaxCatalogController extends Controller {
  ...
  static function routes() {
    ...
    Route::post('/f-export', '\Vaden\AjaxCatalog\AjaxCatalogController@export')
      ->name('catalogExport');

    //handler of bulk operations
    Route::post('/f-bulk-operations',
      '\Vaden\AjaxCatalog\AjaxCatalogController@bulkOperations')
      ->name('catalogBulkOperations');

    //for ajax form fields
    Route::post('/f-ajax-field', '\Vaden\AjaxCatalog\AjaxCatalogController@ajaxField')
      ->name('catalogAjaxField');
    Route::post('/f-ajax-autocomplete',
      '\Vaden\AjaxCatalog\AjaxCatalogController@ajaxAutocomplete')
      ->name('catalogAjaxAutocomplete');
    Route::post('/f-ajax-autocomplete-2',
      '\Vaden\AjaxCatalog\AjaxCatalogController@ajaxAutocomplete2')
      ->name('catalogAjaxAutocomplete2');

    //routes for async filters
    Route::get('/f-ajax-async-filter-single', '\Vaden\AjaxCatalog\AjaxCatalogController@ajaxAsyncFilterSingle')
      ->name('catalogAjaxAsyncFilterSingle');
    Route::get('/f-ajax-async-filter-multiple', '\Vaden\AjaxCatalog\AjaxCatalogController@ajaxAsyncFilterMultiple')
      ->name('catalogAjaxAsyncFilterMultiple');
  }
  ...
  public static function generateFiltersRoutes($entity_alias, $route_alias) {
    ...
    Route::get('/' . $entity_alias . '/{all}',
      '\Vaden\AjaxCatalog\AjaxCatalogController@show')
      ->where(['all' => '.*'])
      ->name($route_alias . '.catalogFilters');
  }
  ...
  public function show(Request $request, $all = '') {
    ...
    if (isset($urlData->settings['additional_info'])) {
      ...
      $ajax_catalog_page = AjaxCatalogPage::where('link', $link)
        ->with('seo')
        ->first();
  ...
  private function getSeo($urlData) {
    ...
    if (isset($urlData->settings['title'])) {
      $seo['title'] = $urlData->settings['title'];
      $seo['h1'] = isset($urlData->settings['h1']) ? $urlData->settings['h1'] : ' H1';
  ...
  private function getWidgetsData($paginator, $urlData, &$pageData) {
    ...
    if ($cache = Cache::tags([
      ...
      $pageData['widgetsDataAsync'] = $cache['widgetsAsync'];
      ...
      foreach ($filters as $widget_key => $widget_value) {
        if (isset($widget_value['widget']) && !isset($widget_value['async_load'])) {
        ...
        elseif (isset($widget_value['async_load'])) {
          //separate async filters
          $widgetsDataAsync[$widget_key] = $widget_value;
        }
      }
      ...
      if ($pageData['cache']) {
        Cache::tags([
          config('app.name'),
          config('app.name') . 'ajax_catalog_widgets_data',
        ])->put($urlData->originUrl, [
          'widgets' => $widgetsData,
          'widgetsAsync' => $widgetsDataAsync,
          'js' => $pageData['data_for_js'],
        ], 40);
      }
      $pageData['widgetsData'] = $widgetsData;
      $pageData['widgetsDataAsync'] = $widgetsDataAsync;
    }
  }
  ...
  *check that user can see necessary page
  */
  private function checkPermission($urlData) {
  ...
  */
  public function ajaxField(Request $request) {
    ...
    $text = str_replace(["\\", '/', '%'], '_', strip_tags($request['text']));
  ...
  */
  public function ajaxAutocomplete2(Request $request) {
    $input = trim($request['q']);
  ...
  /*
   *generate data for async filters with 'single' type of parameter 'async_load'
   */
  public function ajaxAsyncFilterSingle(Request $request) {

    $this->validate($request, [
      'filter_name' => 'required|string|max:248',
      'url' => 'required|string|max:2048',
    ]);

    $urlData = new UrlConverter($request->url);
    $filters = isset($urlData->settings['filters']) ? $urlData->settings['filters'] : [];
    $widgetsData = [];
    $htmlData = [];

    foreach ($filters as $widget_key => $widget_value) {
      if ($request['filter_name'] === $widget_key && isset($widget_value['widget'], $widget_value['async_load'])) {
        $widget = WidgetsFactory::create($urlData, $widget_key, $widget_value['name'], $widget_value['widget']);
        $widgetsData[$widget_key] = $widget->getData();
        if (isset($widget_value['async_blade'])) {
          $htmlData[$widget_key] = view($widget_value['async_blade'], [
            'widgetsData' => $widgetsData[$widget_key],
            'widgetsName' => $widget_value['name'] ?? $widget_key,
          ])->render();
        }
      }
    }
  ...
  /*
   *generate data for async filters with 'multiple' type of parameter 'async_load'
   */
  public function ajaxAsyncFilterMultiple(Request $request) {
    ...
    foreach ($filters as $widget_key => $widget_value) {
      ...
      if (isset($widget_value['widget'], $widget_value['async_load']) && $widget_value['async_load'] === 'multiple') {
        $widget = WidgetsFactory::create($urlData, $widget_key, $widget_value['name'], $widget_value['widget']);
        $widgetsData[$widget_key] = $widget->getData();
        if (isset($widget_value['async_blade'])) {
          $htmlData[$widget_key] = view($widget_value['async_blade'], [
