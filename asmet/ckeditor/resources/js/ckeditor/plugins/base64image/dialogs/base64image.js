...
CKEDITOR.dialog.add("base64imageDialog", function (editor) {
    ...
    function renameImage() {
        if (axios) {
            ...
            let altI = t.getContentElement('tab-properties', 'alt');
            let alt = null;
            try {
                alt = altI.getInputElement().$;
            } catch (e) {
                alt = null;
            }
            let src = urlI.getValue();

            if (!src && imgPreview) {
                src = imgPreview.getElement().$.children[0].src;
            }
            ...
            if (alt.value && src) {
                axios.post('/ckeditor-file-upload', data).then(res => {
                    if(res.data.hasOwnProperty('src')
            )
                {
                    /* Ensable Image URL Checkbox */
                    if (urlCB) urlCB.setValue(true, true);
                    if (fileCB) fileCB.setValue(false, true);
                    urlI.setValue(res.data.src);
                }
            }).
    ...
    function imagePreview(src) {
        ...
        if (src == "base64") {
            ...
            if (n && "files" in n && n.files && n.files.length > 0 && n.files[0]) {
                ...
                let data = new FormData();
                data.append('file[]', n.files[0]);
                data.append('alt', alt.value);

                axios.post('/ckeditor-file-upload', data)
                    .then(res => {
                    imgPreview.getElement().setHtml("");
                imagePreviewLoad(res.data.src);
            })
            .catch(err => imgPreview.getElement().setHtml("")
            )
                ;
    ...
    return {
        ...
        onShow: function () {
            ...
            if (selectedImg) {
                ...
                if (typeof(selectedImg.getAttribute("data-external-link")) == "string") t.setValueOf("tab-properties", "external-link", selectedImg.getAttribute("data-external-link"));
        ...
        onOk: function () {
            ...
            var newImg;
            if (selectedImg) newImg = selectedImg; else newImg = editor.document.createElement("img");
            if (selectedImg) newImg.setAttribute("data-cke-saved-src", src);
            newImg.setAttribute("src", src);
            src = null;

            /* Set attributes */
            let userAlt = t.getValueOf("tab-properties", "alt").replace(/^\s+/, "").replace(/\s+$/, "");
            let userLink = t.getValueOf("tab-properties", "external-link");
            newImg.setAttribute("alt", userAlt);
            newImg.setAttribute("title", userAlt);
            newImg.setAttribute("data-external-link", userLink);
            var attr = {
            ...
            /* Add wrapper to new image */
            let hasWrapper = newImg.getParent();
            let link;
            if(hasWrapper) link = hasWrapper; else link = editor.document.createElement('a');
            if(userLink) {
                link.setAttribute("href", userLink);
                if (hasWrapper) link.setAttribute("data-cke-saved-href", userLink);
                link.setAttribute("class", '');
            } else {
                link.setAttribute("href", newImg.getAttribute("src"));
                if (hasWrapper) link.setAttribute("data-cke-saved-href", newImg.getAttribute("src"));
                link.setAttribute("class", 'image-text-link');
            }
            link.setAttribute("title", userAlt);
			link.setAttribute("rel", 'nofollow');

            let insert;
            if(hasWrapper) insert = newImg; else insert = newImg.appendTo(link);
